...
//Class: CS344
//Program: otp_enc_d
//Description: Gets plaintext and key from client, encrypts it and sends it back to client
...
*/
		...
		if((strcmp(client_name, "otp_enc") != 0))
		{
		...
		//	printf("test 2\n");
			...
			*/
			...
			for(i = 0; i < lenPlainText; i++)
			{
				...
				CryptBuff[i] = ntot((tton(PlainTextBuff[i]) + tton(KeyBuff[i]))%27);
			}
